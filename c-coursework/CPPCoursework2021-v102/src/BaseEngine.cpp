...
BaseEngine::BaseEngine()
    : m_bNotifyObjectsAboutKeys(false)
      ...
      , m_iExitWithCode(-1)
      , m_pKeyStatus(nullptr)
      , m_bNeedsRedraw(true)
      , m_iMouseXClickedDown(0)
      , m_iMouseYClickedDown(0) // don't wait for events before continuing each loop
      ...
      , m_bBlockWaitingForEvents(false)
...
void BaseEngine::virtCreateWindows(const char* szCaption) {
    ...
    // Admittedly making it full screen is something some people may want to do, but not for this coursework please.
    ...
    m_pSDL2Texture = SDL_CreateTexture(m_pSDL2Renderer, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_STREAMING,
                                       m_iWindowWidth, m_iWindowHeight);
}
...
*/
int BaseEngine::initialise(const char* szCaption, int iWindowWidth, int iWindowHeight, const char* szFontName,
                           int iFontSize) {
    ...
    if((szFontName != nullptr) && (strlen(szFontName) > 0))
...
bool BaseEngine::handleEvent(SDL_Event* pEvent) {
    ...
    if(firstClick && (pEvent->type == SDL_MOUSEBUTTONDOWN)) {
        ...
        if((pEvent->button.x == 0) && (pEvent->button.y == 0))
            return true;
        // Skip if this is the rubbish one at the start which occurs when in release mode (but not debug mode) - not sure why!
    }
    ...
    switch(pEvent->type) {
        case SDL_KEYDOWN:
            ...
            if(m_bNotifyObjectsAboutKeys)
                std::for_each(m_vecDisplayableObjects.begin(), m_vecDisplayableObjects.end(),
                              [=](DisplayableObject* pdo) {
                                  if(pdo == nullptr) return;
                                  pdo->virtKeyDown(pEvent->key.keysym.sym);
                              });
            break;
        ...
        case SDL_KEYUP:
            ...
            if(m_bNotifyObjectsAboutKeys)
                std::for_each(m_vecDisplayableObjects.begin(), m_vecDisplayableObjects.end(),
                              [=](DisplayableObject* pdo) {
                                  if(pdo == nullptr) return;
                                  pdo->virtKeyUp(pEvent->key.keysym.sym);
                              });
            break;
        ...
        case SDL_MOUSEMOTION:
            ...
            if(m_bNotifyObjectsAboutMouse)
                std::for_each(m_vecDisplayableObjects.begin(), m_vecDisplayableObjects.end(),
                              [=](DisplayableObject* pdo) {
                                  if(pdo == nullptr) return;
                                  pdo->virtMouseMoved(m_iCurrentMouseX, m_iCurrentMouseY);
                              });
            break;
        ...
        // Note: If you are not using a filter this just assigns the motion.x to the m_iCurrentMouseX. If using a filter it may convert the point. See filter at top of DrawingSurface if you really need to know what this is doing.
            ...
            if(m_bNotifyObjectsAboutMouse)
                std::for_each(m_vecDisplayableObjects.begin(), m_vecDisplayableObjects.end(),
                              [=](DisplayableObject* pdo) {
                                  if(pdo == nullptr) return;
                                  pdo->virtMouseDown(pEvent->button.button, m_iMouseXClickedDown, m_iMouseYClickedDown);
                              });

            if(m_bNotifyBottomObjectAboutMouse) {
                for(int i = 0; i < m_vecDisplayableObjects.size(); i++)
                    if(m_vecDisplayableObjects[i] != nullptr
                        && m_vecDisplayableObjects[i]->virtSingleItemMouseDown(
                            pEvent->button.button, m_iMouseXClickedDown, m_iMouseYClickedDown)) {
            ...
            if(!bObjectHandledClick) {
                ...
                virtBackgroundMouseDown(pEvent->button.button, pEvent->button.x, pEvent->button.y, m_iMouseXClickedDown,
                                        m_iMouseYClickedDown);
            }
            break;
        ...
        // Note: If you are not using a filter this just assigns the motion.x to the m_iCurrentMouseX. If using a filter it may convert the point. See filter at top of DrawingSurface if you really need to know what this is doing.
            ...
            if(m_bNotifyObjectsAboutMouse)
                std::for_each(m_vecDisplayableObjects.begin(), m_vecDisplayableObjects.end(),
                              [=](DisplayableObject* pdo) {
                                  if(pdo == nullptr) return;
                                  pdo->virtMouseUp(pEvent->button.button, m_iMouseXClickedUp, m_iMouseYClickedUp);
                              });
        //redrawDisplay();
            break;
        ...
        case SDL_FINGERDOWN:
        case SDL_FINGERUP:
            virtTouchEvent(pEvent->tfinger.type, pEvent->tfinger.timestamp, pEvent->tfinger.touchId,
                           pEvent->tfinger.fingerId, pEvent->tfinger.x, pEvent->tfinger.y, pEvent->tfinger.dx,
                           pEvent->tfinger.dy, pEvent->tfinger.pressure);
            break;
...
void BaseEngine::virtRenderScreen(void) {
    ...
    else // Partial redraw so only clear the part of screen we are redrawing.
    {
        ...
        this->copyBackgroundPixels(m_iRedrawRealLeft, m_iRedrawRealTop, m_iRedrawRealRight - m_iRedrawRealLeft,
                                   m_iRedrawRealBottom - m_iRedrawRealTop);
